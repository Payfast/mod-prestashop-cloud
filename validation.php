	...
	if ( !$pf_error && !$pf_done)
	{
		...
		switch ( $pf_data['payment_status'])
		{
			case 'COMPLETE':
				...
				$payfast->validateOrder((int)$pf_data['custom_int1'], _PS_OS_PAYMENT_, (float)$total, 
				$payfast->displayName, null, array('transaction_id'=>$transaction_id), null, false, $pf_data['custom_str1']);                
				break;
			...
			case 'FAILED':
				...
				$payfast->validateOrder((int)$pf_data['custom_int1'], _PS_OS_ERROR_, (float)$total, 
				$payfast->displayName, null,array('transaction_id'=>$transaction_id), null, false, $pf_data['custom_str1']);
